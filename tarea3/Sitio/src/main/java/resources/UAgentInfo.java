...
public class UAgentInfo {
	...
	/**
	 * Detects if the current device is a (small-ish) Android OS-based device used
	 * for calling and/or multi-media (like a Samsung Galaxy Player). Google says
	 * these devices will have 'Android' AND 'mobile' in user agent. Ignores tablets
	 * (Honeycomb and later).
	 * 
	 ...
	 * says these devices will have 'Android' and NOT 'mobile' in their user agent.
	 * 
	...
	public boolean detectAndroidTablet() {
		...
		// Otherwise, if it's Android and does NOT have 'mobile' in it, Google says it's
		// a tablet.
		return (userAgent.indexOf(mobile) <= -1);
	}

	/**
	 * Detects if the current device is an Android OS-based device and the browser
	 * is based on WebKit.
	 * 
	...
	/**
	 * Detects if the current browser is a Windows Mobile device. Excludes Windows
	 * Phone 7.x and 8 devices. Focuses on Windows Mobile 6.xx and earlier.
	 * 
	...
	/**
	 * Detects if the current browser is a BlackBerry device AND uses a WebKit-based
	 * browser. These are signatures for the new BlackBerry OS 6. Examples: Torch.
	...
	/**
	 * Detects if the current browser is a BlackBerry device AND has a more capable
	 * recent browser. Excludes the Playbook. Examples, Storm, Bold, Tour, Curve2
	...
	public boolean detectBlackBerryHigh() {
		...
		return !detectBlackBerryWebKit() && detectBlackBerry()
				&& (detectBlackBerryTouch() || userAgent.indexOf(deviceBBBold) != -1
						|| userAgent.indexOf(deviceBBTour) != -1 || userAgent.indexOf(deviceBBCurve) != -1);
	}

	/**
	 * Detects if the current browser is a BlackBerry device AND has an older, less
	 * capable browser. Examples: Pearl, 8800, Curve1
	 * 
	...
	/**
	 *
	 * Detects if the current device is any Symbian OS-based device, including older
	 * S60, Series 70, Series 80, Series 90, and UIQ, or other browsers running on
	 * these devices.
	 * 
	...
	public boolean detectPalmOS() {
		...
		// Blazer or Xiino.
		return userAgent.indexOf(devicePalm) != -1 || userAgent.indexOf(engineBlazer) != -1
	...
	/**
	 * Detects if the current device is an Amazon Kindle (eInk devices only). Note:
	 * For the Kindle Fire, use the normal Android methods.
	 * 
	...
	/**
	 * Detects if the current device is a handheld gaming device with a touchscreen
	 * and modern iPhone-class browser. Includes the Playstation Vita.
	 * 
...
//*****************************
	...
	/**
	 * Detects if the current device is a mobile device. This method catches most of
	 * the popular modern devices. Excludes Apple iPads and other modern tablets.
	 * 
	...
	/**
	 * The longer and more thorough way to detect for a mobile device. Will probably
	 * detect most feature phones, smartphone-class devices, Internet Tablets,
	 * Internet-enabled game consoles, etc. This ought to catch a lot of the more
	...
	public boolean detectMobileLong() {
		...
		if (userAgent.indexOf(manuSonyEricsson) != -1)
			return true;
		return userAgent.indexOf(manuericsson) != -1 || userAgent.indexOf(manuSamsung1) != -1
				|| userAgent.indexOf(svcDocomo) != -1 || userAgent.indexOf(svcKddi) != -1
	 ...
	 * devices which can display iPhone-optimized web content. Includes iPhone, iPod
	 * Touch, Android, Windows Phone 7 and 8, BB10, WebOS, Playstation Vita, etc.
	 * 
