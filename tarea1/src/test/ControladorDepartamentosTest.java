...
class ControladorDepartamentosTest {
	...
	void setUpAfterClass() throws Exception {
		...
		for (Departamento d : md.getDepartamentos().values()) {
			md.removerDepartamento(d.getNombre());
		}
	}
	...
	void testCrearDepartamentoFail() {
		...
		assertThrows(DepartamentoRepetidoException.class, () -> {icd.crearDepartamento(nombre, descripcion, link);});
	}
	...
	void testConsultarDepartamento() {
		Fabrica fabrica = Fabrica.getInstance();
		IControladorDepartamentos icd = fabrica.getIControladorDepartamentos();
		assertThrows(DepartamentoNoExisteException.class, () -> {icd.consultarDepartamento("algo");});
	}
	...
	void testModificarDepartamentoFail() {
		Fabrica fabrica = Fabrica.getInstance();
		IControladorDepartamentos icd = fabrica.getIControladorDepartamentos();
		assertThrows(DepartamentoNoExisteException.class, () -> {icd.modificarDepartamento("algo", "desc", "link");});
