...
public class ConsultarActividad extends JInternalFrame {
	...
	protected boolean desdeConsultarPaquetes = false;
	protected boolean desdeConsultarUsuarios = false;
			...
			public void actionPerformed(ActionEvent event) {
				if (comboBoxActividad.getSelectedItem() != null && comboBoxActividad.getSelectedItem() != " ") {
					...
					try {
						...
						textFieldCosto.setText(Double.toString(dataAct.getReal()));
			...
			public void actionPerformed(ActionEvent event) {
				if (comboBoxPaquetes.getSelectedItem() != null) {
					...
					try {
						...
						ifrCPaq.desdeConsultarActividad = true;
						ifrCPaq.fromConsultarActividad(dpaq);
						ifrCPaq.setVisible(true);
						setVisible(false);
					} catch (Exception e2) {
						JOptionPane.showMessageDialog(btnVerPaquetes, e2.getMessage(), "Consultar Actividad",
								JOptionPane.ERROR_MESSAGE);
					}
				}
			}
			...
			public void actionPerformed(ActionEvent event) {
				if (comboBoxSalidas.getSelectedItem() != null) {
					...
					try {
						...
						ifrcSal.recargar();
						ifrcSal.desdeConsultarActividad = true;
	...
	public void fromOtraVentana(DataActividadTuristica dact) {
		...
		textFieldCosto.setText(Double.toString(dact.getReal()));
