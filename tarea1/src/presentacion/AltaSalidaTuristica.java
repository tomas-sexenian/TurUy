...
public class AltaSalidaTuristica extends JInternalFrame {
	...
	public AltaSalidaTuristica() {
		...
		getContentPane().add(textFieldHora, gbctextFieldC);
	...
	protected void altaSalidaActionPerformed(ActionEvent arg0) {
		...
		Date fAlta = this.dateChooser2.getDate();
		Date fSalida = this.dateChooser.getDate();
		String hor = this.textFieldHora.getText().trim();
		...
		if (nombre.isEmpty() || hor.isEmpty() || salida.isEmpty() || cantidad.isEmpty() || fAlta == null
				|| fSalida == null) {
			JOptionPane.showMessageDialog(this, "No puede haber campos vacíos", "Alta de salida turistica",
					JOptionPane.ERROR_MESSAGE);
		} else {
			try {
				LocalDate fechaAlta = fAlta.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();
				...
				try {
					...
					try {
						...
						if (fechaSalida.isBefore(fechaAlta)) {
							...
							JOptionPane.showMessageDialog(this, "La hora de la salida debe ser un natural entre 0 y 23",
									"Alta de salida turistica", JOptionPane.ERROR_MESSAGE);
